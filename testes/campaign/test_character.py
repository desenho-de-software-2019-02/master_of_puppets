...
base_json = {
  "user": "joao",
  "character_sheet": "id_d_alguma_ficha"
}

def get(id='', port=9000, endpoint='characters'):
    base_url = "http://0.0.0.0:{}/{}/{}".format(port, endpoint, id)
    headers = { 'accept': 'application/json', 'Content-Type' : 'application/json' }
    r = requests.get(base_url, headers=headers)
    return r

def post(data, id='', port=9000, endpoint='characters'):
    base_url = "http://0.0.0.0:{}/{}/{}".format(port, endpoint, id)
    headers = { 'accept': 'application/json', 'Content-Type' : 'application/json' }
    r = requests.post(base_url, data=json.dumps(data), headers=headers)
    return r
    
def put(data, id='', port=9000, endpoint='characters'):
    base_url = "http://0.0.0.0:{}/{}/{}".format(port, endpoint, id)
    headers = { 'accept': 'application/json', 'Content-Type' : 'application/json' }
    r = requests.put(base_url, data=json.dumps(data), headers=headers)
    return r
   
def delete(id='', port=9000, endpoint='characters'):
    base_url = "http://0.0.0.0:{}/{}/{}".format(port, endpoint, id)
...
class TestCampaignMethods(unittest.TestCase): 
    ...
    def test_put(self):
        ...
        out_user = json.loads(get(id=id)._content)

        self.assertEqual(data['user'], out_user['user'])
